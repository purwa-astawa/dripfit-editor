...
access token" field and to embed in the exported `dripfit-config`.
...
## Shared knowledge base — keep the dripfit-config contract in sync

The `dripfit-config` shape is the contract shared with the **Visualizer** (a separate
project). It is documented in the shared knowledge base at
`../shared-knowledge/` (i.e. `DripFitLab/shared-knowledge/`), whose
`dripfit-config-contract.md` mirrors `src/lib/schema.ts`.

- **After committing a change that touches the dripfit-config contract** —
  `src/lib/schema.ts` (`DripfitConfig`, `CalloutProduct`, `parseDripfitConfig`,
  accepted legacy shapes) or the exported shape from `editorStore.exportJson` —
  update `../shared-knowledge/dripfit-config-contract.md` to match and add a changelog
...
exports a `dripfit-config` for the Visualizer widget. Coordinates are stored as ratios
...
- **dripfit-config** — the JSON file the editor exports and the Visualizer
  ...
  data. Defined by `src/lib/schema.ts` (`DripfitConfig` / `parseDripfitConfig`);
  the on-disk file is `dripfit-config.json`. (Formerly called `map.json`.)
